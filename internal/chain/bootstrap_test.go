...
func TestLoadBootstrapInception(t *testing.T) {
	...
	if vb.PredecessorPresent {
		t.Errorf("omitted version_predecessor should be absent, got present: %+v", vb)
	}
}
...
func TestVersionPredecessorShapes(t *testing.T) {
	...
	cases := []struct {
		...
		{"null", `, "version_predecessor": null`, true, true, false, false},
		{"ambiguous list", `, "version_predecessor": [{"tag":"a"},{"tag":"b"}]`, true, false, true, false},
		{"binding", `, "version_predecessor": {"tag":"v1.0.0","ref_oid":"x","commit_oid":"y"}`, true, false, false, true},
	}
	for _, c := range cases {
